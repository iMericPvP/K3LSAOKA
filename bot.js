...
client.on('message', async msg => { 
	...
	if (command === `play`) {
		...
		if (!voiceChannel) return msg.channel.send('عليك دخول روم صوتي');
		const permissions = voiceChannel.permissionsFor(msg.client.user);
		if (!permissions.has('CONNECT')) {
			
			return msg.channel.send('يجب اعطاء البوت صلاحيه دخول هذه الروم');
		}
		if (!permissions.has('SPEAK')) {
			return msg.channel.send('يجب اعطاء البوت صلاحيه التكلم في هذه الروم');
		}

		if (!permissions.has('EMBED_LINKS')) {
			return msg.channel.sendMessage("**يجب اعطاء البوت صلاحيه EMBED_LINKS**")
		}
		...
		if (url.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/)) {
			...
			return msg.channel.send(` **${playlist.title}** تم اضافه الي قائمه التشغيل بنجاح`);
		} else {
			try {
				...
				try {
			        ...
			        .setDescription(`**يجب عليك اختيار رقم المقطع** :
${videos.map(video2 => `[**${++index} **] \`${video2.title}\``).join('\n')}`)
					...
					try {
						...
						console.error(err);
						return msg.channel.send('لم يتم اختيار فيديو');
					}
					const videoIndex = parseInt(response.first().content);
					var video = await youtube.getVideoByID(videos[videoIndex - 1].id);
				} catch (err) {
					console.error(err);
					return msg.channel.send('البوت لايجد فيديو بهذه الاسم');
				}
			}
		...
		if (!serverQueue) return msg.channel.send('لا يوجد فديوي لتغطيه');
		serverQueue.connection.dispatcher.end('تم تجاوز المقطع بنجاح');
		return undefined;
	} else if (command === `stop`) {
		if (!msg.member.voiceChannel) return msg.channel.send('أنت لست بروم صوتي ');
		if (!serverQueue) return msg.channel.send('لا يوجد مقطع لي ايقافه');
		serverQueue.songs = [];
		serverQueue.connection.dispatcher.end('تم ايقاف هذه المقطع بنجاح');
		return undefined;
	} else if (command === `vol`) {
		if (!msg.member.voiceChannel) return msg.channel.send('أنت لست بروم صوتي ');
		if (!serverQueue) return msg.channel.send('لا يوجد مقطع لي ترفيع صوته');
		if (!args[1]) return msg.channel.send(`:loud_sound: تم تغير مستوي صوت **${serverQueue.volume}**`);
		serverQueue.volume = args[1];
		serverQueue.connection.dispatcher.setVolumeLogarithmic(args[1] / 50);
		return msg.channel.send(`:speaker: تم تغيير الصوت الي **${args[1]}**`);
	} else if (command === `np`) {
	...
	.setDescription(`:notes: الان يتم تشغيل : **${serverQueue.songs[0].title}**`)
		...
		if (!serverQueue) return msg.channel.send('لا يوجد مقطع');
		let index = 0;
		
		const embedqu = new Discord.RichEmbed()

.setDescription(`**الاااني**
...
**الان يتم تشغيل** ${serverQueue.songs[0].title}`)
		...
		if (serverQueue && serverQueue.playing) {
			...
			return msg.channel.send('تم ايقاف الفديو مؤقت');
		}
		return msg.channel.send('لا يوجد مقطع');
